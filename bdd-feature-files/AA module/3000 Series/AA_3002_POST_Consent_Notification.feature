Feature: On calling POST Consent Notification API, verify that on sending consent
  notification request with basic invalid values for each required field,
  the error response is received.

  Scenario: 3002_1 On calling POST Consent Notification API, use the pre-linked user details
  from settings. Set the POST consent response in FIP mock server. Send a valid consent request
  and ask the user to approve the consent and confirm.Send a valid consent handle to receive
  APPROVED status. Send a consent fetch request and finally send a consent notification request
  with each time one required field set to null and Verify that the error response is received.
  ...
  notification request with each time one required field set to empty string and Verify that the error response is received.
    Given Calling the "POST /Consent/Notification" Flow API
    When  POST action is performed
    Then  Verify that the timestamp has the exact format i.e the timestamp is in "+15" or "-15" minutes from current
  time stamp
  ...
  consent response in FIP mock server. Send a valid consent request and ask the user to approve the consent and confirm.
  Send a valid consent handle to receive APPROVED status. Send a consent fetch request and finally send a consent
  notification request with each time one enum field set to non enum value. Add an extra character at the
  end of last and Verify that the error response is received.
