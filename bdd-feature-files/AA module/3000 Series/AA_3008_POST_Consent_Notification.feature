Feature: On calling POST Consent Notification API, verify that on sending consent
  notification request as alternate FIP with consent details of regular FIP,
  the error response is received.

  Scenario: 3008_1 On calling POST Consent Notification API, set the POST consent response in FIP mock server. Send a
  valid consent request and ask the user to approve the consent and confirm. Send a valid consent handle to receive
  APPROVED status. Send a consent fetch request and finally send a consent notification request. Send the consent
  ...
  Scenario: 3008_2 On calling POST Consent Notification API, set the POST consent response in FIP mock server. Send a
  valid consent request and ask the user to approve the consent and confirm. Send a valid consent handle to receive
  APPROVED status. Send a consent fetch request and finally send a consent notification request. Use the consent details of
