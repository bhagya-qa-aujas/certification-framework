Feature: On calling FIP and FIU Notifications API, verify that on FIP responding with
  invalid session id format for FI request from AA, FI notification with error status is sent back to FIU.

  Scenario: 2024_1 On calling FIP and FIU Notifications API, use the pre-generated consent details from settings and
  ...
  Scenario: 2024_2 On calling FIP and FIU Notifications API, use the pre-generated consent details from settings and
  set the invalid values for FI response in mock FIP. Set the FI notification response in FIU mock server and send a
