Feature: On calling FIP and FIU Notifications API, verify that on FIP responding with non
  matching txnid for FI request from AA, FI notification with error status is sent back to FIU.

  Scenario: 2022_1 On calling FIP and FIU Notifications API, use the pre-generated consent details from settings and set the
  invalid values for FI response in mock FIP. Set the FI notification response in FIU mock server and send a valid FI
  request to AA. Wait to receive FI notification in mock FIU. Generate a new UUID and use that.
