Feature: On calling POST FI Request API, verify that on sending FI request with invalid consent
  digitalSignature,the error response is received.

  Scenario: 2009_1 On calling the POST FI Request API, use the pre-generated consent details from settings. Use the
  valid consent digitalSignature and add an extra character at the end and Verify that the HTTP status code 400 is received.
  ...
  valid consent digitalSignature and remove the last character and Verify that the HTTP status code 400 is received.
    Given Calling the "POST /FI/request" Flow API
    When  POST action is performed
    Then  Verify that the response code displayed is HTTP 400
    And   Verify that the timestamp has the exact format i.e the timestamp is in "+15" or "-15" minutes from current
  ...
  pre-generated consent full object from the same setting and generate a proper digitalSignature
  using the test key and Verify that the HTTP status code 400 is received
    Given Calling the "POST /FI/request" Flow API
    When  POST action is performed
    Then  Verify that the response code displayed is HTTP 400
    And   Verify that the timestamp has the exact format i.e the timestamp is in "+15" or "-15" minutes from current
