Feature: On calling POST FI Request API, verify that on sending FI request when consent is
  paused,error response is received.

  Scenario: 2012_1 On calling the POST FI Request API, Use the pre-linked user details
  from settings, send a valid consent request, ask the user to approve the consent & confirm,
  send a valid consent handle to receive APPROVED status, ask the user to pause the consent &
  confirm, send a consent fetch request to receive status as PAUSED and then send a FI request
