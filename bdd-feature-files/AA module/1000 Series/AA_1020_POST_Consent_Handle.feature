Feature: On calling POST Consent Handle request API,Verify that on sending consent
  handle request for consent that is paused by user, response with consent status
  APPROVED is received.

  Scenario: 1020_1 On calling POST Consent Handle request API,Use the pre-linked user
  ...
  APPROVED status, ask the user to pause the consent & confirm and finally send the
