Feature: On Calling POST FI Request with invalid JWS API signature, error response is received.
  ...
  Scenario: 3020_1 Use the pre-linked user details from settings, set Consent/notification response in mock
  ...
  Scenario: 3020_2 Use the pre-linked user details from settings, set Consent/notification response in mock
  server, send a valid consent request and then send a valid FI request with following variations on the
  x-jws-signature header. Send POST Account FIRequest with valid details by Using the valid JWS signature but
  ...
  Scenario: 3020_3 Use the pre-linked user details from settings, set Consent/notification response in mock
  server, send a valid consent request and then send a valid FI request with following variations on the
  ...
  Scenario: 3020_4 Use the pre-linked user details from settings, set Consent/notification response in mock
  server, send a valid consent request and then send a valid FI request with following variations on the
