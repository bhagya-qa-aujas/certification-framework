Feature: On Calling POST FI Request for consents of different consentModes and check if it is
  ...
  Scenario: 3012_1 Use the pre-linked user details from settings and try the following
  ...
  Scenario: 3012_2 Use the pre-linked user details from settings and try the following
  1. Set Consent/notification response in mock server, Post consent with each valid consentMode with dataLife
  value set to 0, except for consentMode STORE dataLife should be greater than 0 and then send the FI request
  ...
  Scenario: 3012_3 Use the pre-linked user details from settings and try the following
  1. Set Consent/notification response in mock server, Post consent with each valid consentMode with dataLife
  value set to 0, except for consentMode STORE dataLife should be greater than 0 and then send the FI request
  ...
  Scenario: 3012_4 Use the pre-linked user details from settings and try the following
  1. Set Consent/notification response in mock server, Post consent with each valid consentMode with dataLife
  value set to 0, except for consentMode STORE dataLife should be greater than 0 and then send the FI request
