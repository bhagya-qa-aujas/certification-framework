Feature: On Calling FI Fetch request with valid details and check if it is responding with
  ...
  Scenario: 3009_1 Use the pre-linked user details from settings, set Consent/notification response in mock
  ...
  request with valid details and Verify that HTTP 200 is displayed in response.
