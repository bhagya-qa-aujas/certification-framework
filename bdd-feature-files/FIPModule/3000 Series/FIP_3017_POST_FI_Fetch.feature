Feature: On Calling FI Fetch request for consent that is not active and check if it is responding
  ...
  Scenario: 3017_1 Use the pre-linked user details from settings, set Consent/notification response in mock
  ...
  the consent notification and Verify that HTTP 403 is displayed in response.
  ...
  Scenario: 3017_2 Use the pre-linked user details from settings, set Consent/notification response in mock
  server, send one valid consent post request. Set Fi/notification response on mock server. send FI request,
  wait for FI Ready notification and then try the following. Send FI Fetch request after expiring consent
  using the consent notification and Verify that HTTP 403 is displayed in response.
    Given Calling the "POST /FI/fetch" Flow API
    When  POST action is performed
    Then  Verify that the Response code 403 is displayed
    And   Verify that the timestamp has the exact format i.e the timestamp is in "+15" or "-15" minutes from the current time stamp
  ...
  Scenario: 3017_3 Use the pre-linked user details from settings, set Consent/notification response in mock
  server, send one valid consent post request. Set Fi/notification response on mock server. send FI request,
  wait for FI Ready notification and then try the following. Send FI Fetch request after revoking consent
  using the consent notification and Verify that HTTP 403 is displayed in response.
    Given Calling the "POST /FI/fetch" Flow API
    When  POST action is performed
    Then  Verify that the Response code 403 is displayed
  ...
  Scenario: 3017_4 Use the pre-linked user details from settings, set Consent/notification response in mock
  server, send one valid consent post request. Set Fi/notification response on mock server. send FI request,
  ...
  request for the expired consent and Verify that HTTP 403 is displayed in response.
  GGiven  Calling the "POST /FI/fetch" Flow API
    Given Calling the "POST /FI/fetch" Flow API
    When  POST action is performed
    Then  Verify that the Response code 403 is displayed
    And   Verify that the timestamp has the exact format i.e the timestamp is in "+15" or "-15" minutes from the current time stamp
