Feature: On Calling POST Account discovery API with valid customer id, all combinations of supported FITypes
  ...
  Scenario: 1022_1 On Calling POST Account discovery with Strong Identifier MOBILE and FITypes DEPOSIT,
  TERM_DEPOSIT and expecting HTTP status code 200 in response.
  ...
  Scenario: 1022_2 On Calling POST Account discovery with Strong Identifier MOBILE and FITypes DEPOSIT,
  ...
  Scenario: 1022_3 On Calling POST Account discovery with Strong Identifier MOBILE and FITypes TERM_DEPOSIT,
  RECURRING_DEPOSIT and expecting HTTP status code 200 in response.
    Given Calling the "POST /Accounts/discover" Flow API
    When  POST action is performed
    Then  Verify that Number of items in the list, accType and accRefNumber for each should match as per the test
