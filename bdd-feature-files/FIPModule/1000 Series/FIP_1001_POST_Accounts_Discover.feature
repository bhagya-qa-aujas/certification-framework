Feature: On Calling POST Account discovery API with valid customer id, FIType as DEPOSIT and strong customer
  ...
  in DEPOSIT FIType and expect the received account details to have exactly one account of DEPOSIT type, account
  numbers to be masked properly and account details to exactly match the user DEPOSIT accounts.
    ...
    Then  Verify that number of items in the list, accType and accRefNumber for each should match as per the test user data from settings
