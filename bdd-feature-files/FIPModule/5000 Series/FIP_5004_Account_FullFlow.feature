Feature:On calling Full Flow APIs except account discovery and consent notification, Send the account link
  ...
  supported FITypes and check that all APIs are responding with success response and Verify that the response code
  of all APIs is 200.

  Scenario: 5004_1 On calling Full Flow APIs, send the request with first account of user having three account
  for All FIType and Verify that the HTTP status code 200 is received in response.
  ...
  Scenario: 5004_3 On calling Full Flow APIs, send the request  with user having three account for All FIType and
