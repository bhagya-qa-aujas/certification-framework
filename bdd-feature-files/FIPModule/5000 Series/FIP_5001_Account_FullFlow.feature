Feature:On calling Full Flow APIs except account discovery and consent notification, Send the account link
  ...
  Scenario: 5001_1 On calling Full Flow APIs, send the request for single account user, the FIType as DEPOSIT and
  ...
  Scenario: 5001_2 On calling Full Flow APIs, send the request for single account user, the FIType as TERM_DEPOSIT and
  Verify that the HTTP status code 200 is received in response.
    Given Calling the Full Flow APIs
    When  Full Flow APIs action is performed
    Then  Verify that the reponse code of all APIs is 200
  ...
  Scenario: 5001_3 On calling Full Flow APIs, send the request for single account user, the FIType as
  RECURRING_DEPOSIT and Verify that the HTTP status code 200 is received in response.
    Given Calling the Full Flow APIs
    When  Full Flow APIs action is performed
    Then  Verify that the reponse code of all APIs is 200
    And   Verify that the Account link response AuthenticatorType is TOKEN(If TOKEN is not specified in supported
