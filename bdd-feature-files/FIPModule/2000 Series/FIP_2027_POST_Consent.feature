Feature: On Calling POST Consent request as regular AA with valid signedConsent of alternate AA
  ...
  Scenario: 2027_1 Use a single user account with first supported FIType.Do an account discover request as
  alternate AA, send a valid account link request for discovered account as alternate AA, get the received
  OTP from user, send a valid account link verify request as alternate AA. Generate one valid consent post
